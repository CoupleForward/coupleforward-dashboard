...
export function VideoWindowCard() {
  ...
  // One container, one fixed aspect, both states. The card must never
  // change size when Play is tapped — a height change reflows every card
  // below it (Christian, 2026-08-13). The 16:9 video sits letterboxed
  // inside the wide window while playing.
  return (
    <Card padded={false} className="overflow-hidden">
      <div className="relative aspect-[21/7] min-h-[150px] overflow-hidden bg-black">
        {playing ? (
          <iframe
      ...
      <div className="flex items-center justify-between gap-3 px-5 sm:px-8 py-2.5">
        {playing ? (
          <button
            type="button"
          ...
          <span className="text-[11.5px] text-cream-dim">{video.title}</span>
        )}
        <Link
          href="/videos"
          className="shrink-0 inline-flex items-center gap-1 text-[11.5px] font-medium text-gold hover:text-gold-bright transition"
        >
